"use client";

import { useEffect, useState } from "react";
import { useAuth } from "@/components/auth-provider";
import { getBrowserSupabase } from "@/lib/supabase-browser";
import Link from "next/link";

/**
 * Shows an "Upravit" (Edit) button on property detail page
 * only if the logged-in user is the broker who owns this property
 * or owns the agency that owns it.
 */
export function EditPropertyButton({ propertyId }: { propertyId: string }) {
  const { user } = useAuth();
  const [canEdit, setCanEdit] = useState(false);

  useEffect(() => {
    if (!user || !propertyId) return;
    const supabase = getBrowserSupabase();
    if (!supabase) return;

    (async () => {
      // Check if property belongs to this user (as broker or via agency)
      const { data: prop } = await supabase
        .from("properties")
        .select("broker_id, created_by")
        .eq("id", propertyId)
        .single();

      if (!prop) return;

      // Direct creator
      if (prop.created_by === user.id) { setCanEdit(true); return; }

      // User is the broker
      const { data: broker } = await supabase
        .from("brokers")
        .select("id, agency_id")
        .eq("user_id", user.id)
        .single();

      if (!broker) return;

      if (prop.broker_id === broker.id) { setCanEdit(true); return; }

      // User's agency owns the property's broker
      if (broker.agency_id && prop.broker_id) {
        const { data: propBroker } = await supabase
          .from("brokers")
          .select("agency_id")
          .eq("id", prop.broker_id)
          .single();
        if (propBroker?.agency_id === broker.agency_id) { setCanEdit(true); return; }
      }

      // User owns the agency
      const { data: agency } = await supabase
        .from("agencies")
        .select("id")
        .eq("user_id", user.id)
        .single();

      if (agency && prop.broker_id) {
        const { data: propBroker } = await supabase
          .from("brokers")
          .select("agency_id")
          .eq("id", prop.broker_id)
          .single();
        if (propBroker?.agency_id === agency.id) { setCanEdit(true); return; }
      }
    })();
  }, [user, propertyId]);

  if (!canEdit) return null;

  return (
    <Link
      href={`/dashboard/moje-inzeraty/${propertyId}/upravit`}
      className="detail-edit-btn"
      style={{
        display: "inline-flex", alignItems: "center", gap: 6,
        padding: "6px 14px", borderRadius: 8,
        background: "var(--color-accent, #ffb800)", color: "#000",
        fontWeight: 600, fontSize: "0.82rem", textDecoration: "none",
        transition: "opacity 0.15s",
      }}
    >
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
        <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
      </svg>
      Upravit
    </Link>
  );
}
