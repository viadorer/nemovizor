#!/usr/bin/env node
// ============================================================
// Bienici.com Scraper — Côte d'Azur / French Riviera
// Educational / study purposes only
// Fetches global results sorted by price desc, filters client-side
// for Alpes-Maritimes (06xxx) and Var (83xxx) postal codes
// Usage: node scripts/scrape-bienici-riviera.mjs [--pages 100] [--delay 800] [--offset 0]
// ============================================================
...
// Côte d'Azur postal code prefixes
const RIVIERA_PREFIXES = ["06", "83"];

function isRiviera(postalCode) {
  if (!postalCode) return false;
  return RIVIERA_PREFIXES.some(p => postalCode.startsWith(p));
}
...
// ===== State =====
const STATE_FILE = resolve(ROOT, "scripts/.scrape-bienici-riviera-state.json");
function loadState() {
...
async function main() {
  console.log(`
  Bienici.com RIVIERA Scraper | Pages: ${PAGES} | Offset: ${PAGE_OFFSET} | Delay: ${DELAY_MS}ms
  Filter: Côte d'Azur (06xxx Alpes-Maritimes + 83xxx Var)
  Scanning global results, keeping only Riviera properties
`);
  ...
  for (const search of searches) {
    ...
    for (let page = 0; page < pps; page++) {
      ...
      // Filter to Riviera only
      const rivieraAds = ads.filter(ad => isRiviera(ad.postalCode));
      const nonRiviera = ads.length - rivieraAds.length;
      state.stats.filtered += nonRiviera;

      console.log(`\n  Page ${page + 1}/${pps} — ${rivieraAds.length}/${ads.length} Riviera`);
      ...
      for (const ad of rivieraAds) {
        ...
        try {
          ...
          const r2Urls = [];
          const imgSlug = slugify(ad.city || "riviera");
  ...
  console.log(`\n  DONE in ${totalMin}m | Riviera Props: ${state.stats.properties} | Images: ${state.stats.images} | Filtered out: ${state.stats.filtered}\n`);
}
